...
fn answer(id: RequestId, method: &str, params: &Value) -> Vec<Frame> {
    match method {
        "initialize" => vec![
            ...
            // server-initiated message to the agent, and a `listChanged` right
            // after the handshake is the cheapest way to make it do so.
...
mod tests {
    ...
    fn the_handshake_answers_and_then_speaks_unprompted() {
        let frames = answer(RequestId::Number(1), "initialize", &json!({}));
